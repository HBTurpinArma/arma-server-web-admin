...
var fs = require('fs')
const server = require('../public/js/app/models/server')
...
Manager.prototype._addServer = function (data) {
  ...
  if (data.game_selected == "arma3") {
    var server = new Arma3Server(this.config, this.logs, data, this)
  } else if (data.game_selected == "reforger") {
    var server = new ReforgerServer(this.config, this.logs, data, this)
  } else {
...
Manager.prototype.getUsedUIDs = function (config) {
  var self = this
  var uids = []
  self.getServers().map(function (server) {
    if (server.title != config.title & server.port != config.port) {uids.push(server.uid)}
  })
  return uids
}
