...
function copyUserConfig (config, serverFolder, cbaConfigurationOptions) {
  if (config.game == "arma3" || config.game == "arma3_x64") {
...
function createModFolders (config, serverFolder, mods, mods_server_only) {
  // Create virtual folders from default Arma and mods
  const configFolders = (config.virtualServer && config.virtualServer.folders) || []
  const serverMods = (config.serverMods + mods_server_only) || []
  ...
  return fs.promises.mkdtemp(path.join(os.tmpdir(), 'arma-server-'))
    .then((serverFolder) => {
      ...
      return Promise.all([
        ...
        createModFolders(config, serverFolder, server.mods, server.mods_server_only)
      ]).then(() => {
        return serverFolder
      })
    })
