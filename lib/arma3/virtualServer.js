...
function copyKeys (config, game, serverFolder, mods) {
  ...
  if (server.game_selected == "arma3" || server.game_selected == "arma3_x64") {
    ...
    return fs.promises.mkdir(ucFolder, { recursive: true }).then(() => {
      ...
      const defaultUserConfigPromise = fs.promises.readdir(defaultUserConfigPath).then((files) => files.map((file) => path.join(defaultUserConfigPath, file)))
        ...
        fs.promises.copyFile(path.join(defaultUserConfigPath, 'cba_settings.sqf'), path.join(ucFolder, 'cba_settings.sqf')).then(() => {
          fs.promises.appendFile(path.join(ucFolder, "cba_settings.sqf"), server.cbaConfigurationOptions || "")
        }).catch((err) => {
          console.error('Error copying cba_settings.sqf:', err)   
        }) 
