...
Logs.prototype.canUserView = function(user) {
  return this.config.auth[user].permissions.logs.view || false
}

Logs.prototype.canUserDelete = function(user ){
  return this.config.auth[user].permissions.logs.delete || false
}
...
Logs.prototype.logsPath = function () {
  ...
  if (this.config.type === 'windows') {
    ...
    return path.join(this.config.path, 'Profiles')
